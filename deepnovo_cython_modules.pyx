...
def get_location(peptide_mass, prefix_mass, direction):
  ...
  # ion locations
  location_sub50 = np.rint(ion_mass * SPECTRUM_RESOLUTION).astype(np.int32)
...
#~     spectrum_holder_view[neutral_mass_location_view[index]] += norm_intensity_view[index]
  ...
  complement_mass_location = np.rint(complement_mass * deepnovo_config.SPECTRUM_RESOLUTION).astype(np.int32)
